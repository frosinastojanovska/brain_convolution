...
def test_brain_convolution_model(test_subject_ids, trained_model):
    ...
    test_dataset = ADHD('ADHD200_CC200', test_subject_ids,
                        data.loc[data['network_name'].isin(test_subject_ids)], stats)

    data_adj = []
    data_features = []
    for subject_id in test_subject_ids:
        adj, features, classes = test_dataset.get_brain_graph_and_class(subject_id)
        data_features.append(features)
        data_adj.append(adj)
    predictions = trained_model.predict([np.array(data_features), np.array(data_adj)])
    return predictions
...
if __name__ == '__main__':
    ...
    model.summary()
    model.load_weights('trained/model/autoencoder-90-0.18.h5')
    optimizer = Adam(0.001, amsgrad=True, decay=config.WEIGHT_DECAY)
    model.compile(loss='categorical_crossentropy',
                  optimizer=optimizer,
                  metrics=['accuracy'])
